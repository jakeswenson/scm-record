            ...
            if child.kind() == "body" {
                ...
                for block in child.children(&mut body_cursor) {
                    ...
                    if let Some(type_field) = block.child_by_field_name("type") {
                        eprintln!("    Has 'type' field: {:?}", type_field.utf8_text(source.as_bytes()));
                    }
...
"#;
        ...
        assert!(matches!(containers[0].container.kind, ContainerKind::Variable));
    }
...
"#;
        ...
        assert!(matches!(containers[0].container.kind, ContainerKind::Output));
    }
...
"#;
        ...
        assert!(matches!(containers[1].container.kind, ContainerKind::Resource { .. }));

        assert_eq!(containers[2].container.name, "public_ip");
        assert!(matches!(containers[2].container.kind, ContainerKind::Output));
    }
