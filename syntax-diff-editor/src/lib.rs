...
pub fn process_opts(
  ...
  if effective_dir_diff && !opts.dir_diff {
    debug!(
      "Auto-detected directory diff mode: both {} and {} are directories",
      opts.left.display(),
      opts.right.display()
    );
  }
...
pub fn run(opts: Opts) -> Result<()> {
  ...
  debug!(
    "TTY status: stdin={}, stdout={}, stderr={}",
    is_stdin_tty, is_stdout_tty, is_stderr_tty
  );
