...
pub fn create_file(
  ...
  let (old_text, new_text) = match (&left_contents, &right_contents) {
    (FileContents::Text { contents: old, .. }, FileContents::Text { contents: new, .. }) => {
  ...
  if let (Some(ref old), Some(ref new)) = (old_text, new_text) {
    ...
    if let Some(ref containers) = file.containers {
      debug!(
        "Successfully added {} semantic container(s) to {:?}:",
        containers.len(),
        file.path
      );
      for container in containers {
        match container {
          ...
          scm_record::SemanticContainer::Impl {
            type_name,
            trait_name,
            children,
            ..
          ...
          scm_record::SemanticContainer::Function {
            name,
            section_indices,
            ..
          } => {
            debug!(
              "  - Function '{}' with {} section(s)",
              name,
              section_indices.len()
            );
          }
          ...
          scm_record::SemanticContainer::Enum {
            name,
            section_indices,
            ..
          } => {
          ...
          scm_record::SemanticContainer::Object {
            name,
            section_indices,
            ..
          } => {
            debug!(
              "  - Object '{}' with {} section(s)",
              name,
              section_indices.len()
            );
          }
          scm_record::SemanticContainer::Module {
            name,
            section_indices,
            ..
          } => {
            debug!(
              "  - Module '{}' with {} section(s)",
              name,
              section_indices.len()
            );
          }
          scm_record::SemanticContainer::Section {
            name,
            level,
            section_indices,
            ..
            ...
            debug!(
              "  - Section '{}' (level {}) with {} section(s)",
              name,
              level,
              section_indices.len()
            );
          }
          scm_record::SemanticContainer::Method {
            name,
            section_indices,
            ..
          } => {
            debug!(
              "  - Method '{}' with {} section(s)",
              name,
              section_indices.len()
            );
          }
          scm_record::SemanticContainer::Field {
            name,
            section_indices,
            ..
          } => {
            debug!(
              "  - Field '{}' with {} section(s)",
              name,
              section_indices.len()
            );
          }
...
pub fn create_merge_file(
  ...
  let (base_contents, left_contents, right_contents) =
    match (base_contents, left_contents, right_contents) {
      (FileContents::Absent, _, _) => return Err(Error::MissingMergeFile { path: base_path }),
      (_, FileContents::Absent, _) => return Err(Error::MissingMergeFile { path: left_path }),
      (_, _, FileContents::Absent) => return Err(Error::MissingMergeFile { path: right_path }),
